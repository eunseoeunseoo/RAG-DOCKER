...
| Block 4 — QueryEngine | `index.as_query_engine(similarity_top_k=5)` + Mock LLM |
...
|----------|----------|-----------|------------|------------|:--------------:|
| `SummaryIndex` → `VectorStoreIndex` | 전체 문서 | "성능 요구사항 관련 내용은?" | 전체 노드 순차 전달로 LLM 부담 큼, 관련 없는 내용 혼입 | 임베딩 유사도 상위 5개 노드만 전달, 정확도·속도 개선 | 상 |
...
| `similarity_top_k=5` 설정 | 전체 문서 | "Policy.pdf에서 5조 내용은?" | 관련 없는 문서 내용 혼입 | 유사도 상위 5청크로 검색 범위 제한, 노이즈 감소 | 중 |
